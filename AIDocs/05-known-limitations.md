...
- Spheres, emissive sphere lights, emissive mesh lights, registered triangle meshes, and one finite procedural water surface are ray traced. Directional illumination currently uses virtual sun-triangle proposals rather than intersectable sun geometry.
...
- Opaque shading has shared BRDF evaluation, diffuse/GGX mixture-PDF calculation, VNDF sampling, and finite-light/environment MIS. T1's narrow-lobe density defect is repaired using stable cross-product `sin^2(theta)` GGX D evaluation without a denominator floor, with focused GPU BRDF/PDF and normal-incidence sampling-frequency coverage. Grazing-frequency integration and broader acceptance remain pending; final-bounce MIS, proposal consistency, and other estimator defects remain open in document 27. Water and dielectric shortcuts are not equivalent to this opaque BRDF model.
...
- Active wavefront stages have image/queue bounds checks; the retired `CSMain` out-of-bounds claim is not a current blocker. Per-path bounce-budget enforcement and debug hit preservation still have open wavefront audit findings.
