...
- Procedural water is a single global axis-aligned X/Z region intersected by fixed-step ray marching plus binary refinement. It does not support multiple/transformed water bodies, and pathological/high-frequency crossings can be missed. Underwater segment-distance estimation is approximate, especially across the finite side bounds or when a segment enters and exits water between two above-water endpoints. CPU autofocus tests the average water plane rather than the animated wave surface.
