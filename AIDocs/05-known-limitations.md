...
- Spheres, emissive sphere lights, emissive mesh lights, analytic directional lights, registered triangle meshes, and one finite procedural water surface are ray traced.
...
- Direct light sampling supports three strategies (all lights, uniform random, importance-sampled). The random and importance strategies are noisier per frame; final-color frame accumulation can average that variance when the camera and scene are static, but there is still no spatial denoiser. The importance-sampled strategy uses the standard local RIS path for eligible primary opaque direct-light events and ordinary direct sampling for unsupported events and later bounces. ImportanceSampled is unbiased only while every considered light has nonzero selection probability. It currently weights at most `MaxImportanceLights` (`128`) entries, so lights beyond that count are omitted and the result is biased relative to the full scene (a one-time warning is logged). Its importance weight also ignores shadows and the surface normal, so it estimates rather than predicts the true contribution.
...
- Mesh UVs, interpolated vertex normals, imported tangents, albedo maps, glTF-channel metallic/roughness maps, and tangent-space normal maps are supported. Textures remain mesh-only; each material channel's array uses its largest source dimensions, smaller textures are resampled into bilinear slices, mipmaps are not generated, wrapping repeats unconditionally, and source textures must be CPU-readable. Texture contents mutated in place are not detected automatically.
...
- Local direct-light RIS and temporal RIS reuse are part of the standard importance-sampled renderer path. Primary opaque direct-light events draw the configured candidate set and evaluate one selected visibility path; temporal reuse is enabled for supported temporal rendering. Candidate count and temporal reuse state participate in accumulation/history invalidation and benchmark metadata.
...
- `RayTracingShaderPrecompiler` (`Tools > Ray Tracing > Precompile Compute Shader`) was added to force-compile and dispatch the bounded `CSMain` debug/fog/terrain variant matrix from edit mode with per-variant timing and surfaced compile messages, so slow/failing kernels appear there instead of hanging Unity on first Play.
- Fog is disabled by default. When a registered fog volume is active, `FOG_ENABLED` selects a separate variant so ordinary scenes do not compile the full participating-media path. Terrain is similarly isolated behind `TERRAIN_ENABLED`; caustic camera gathering remains runtime-controlled to avoid another variant dimension.
