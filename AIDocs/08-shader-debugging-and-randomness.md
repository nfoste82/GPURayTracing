...
The sampler follows Burley's shuffled-scrambled construction. Every four-dimensional Sobol block receives a stable nested sample-index shuffle, implemented with the prefix-preserving Owen/Laine-Karras-style permutation, before the corresponding Sobol coordinates are evaluated. Each coordinate then receives an independent Owen scramble. Reusing four-dimensional blocks with distinct shuffles retains useful low-dimensional stratification while decorrelating padding across the renderer's large semantic dimension space. Dimensions above the configured limit use the unbiased hash fallback.
