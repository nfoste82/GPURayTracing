...
`GameManager` exposes these controls under **Sampling and Accumulation > Path Sampler**: `useOwenScrambledSobol`, `sobolDimensionLimit`, `samplingSeed`, and `randomNoise`. Owen-Sobol is enabled by default. The direction-number resource covers 2568 dimensions, enough for all semantic dimensions at the maximum 16-bounce setting. The default limit is 328, covering camera plus the first two path bounces; later bounces use the faster hash fallback. Set the limit to 2568 when equal-sample convergence is more important than throughput. When `randomNoise` is false, C# sends the fixed scramble seed while `_SampleOffset` advances to provide deterministic but distinct samples each rendered frame. When `randomNoise` is true, C# additionally sends a new random integer seed each frame. These controls select unbiased sample sequences, so changing them retains progressive accumulation; the image may simply exhibit a changed noise pattern while it converges.

One remaining sequence-preserving optimization candidate is a narrowly scoped `rand2()` helper for unconditional adjacent two-dimensional samples such as pixel jitter, lens disks, area-light disks, and selected BSDF samples. It could share range checks and state transitions while producing exactly the same two coordinates and final `RngState` as two scalar `rand()` calls. Any implementation must fall back to scalar-equivalent handling when the pair crosses a four-dimensional shuffle block or the configured Sobol limit, and should be retained only after raw-coordinate parity, register pressure, shader compile time, and representative wall-clock captures are measured.
