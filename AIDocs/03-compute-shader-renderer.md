...
- `GameManager.enableAdaptiveSampling`: experimental static-final-color progressive path. It schedules full-resolution pixels in independent `8x8` groups at a configurable cadence. Linear-RGB Welford standard-error scores map groups into 16 global urgency buckets. Low-priority tiers receive deterministic rotating service every 2-4 scheduler epochs while higher tiers are eligible every epoch; every bucket with eligible work receives a reserved complete group quantum before remaining budget is assigned by urgency. This avoids permanent low-tier starvation without requiring every pixel to receive one path every frame. Root paths are traced through one indirect dispatch and resolved once per active pixel. `CSMain` remains the unchanged uniform reference when the toggle is off.
