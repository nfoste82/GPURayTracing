...
- `_LightSamplingStrategy`: selects how `GetLightHittingPoint()` samples scene lights (`0` = all lights, `1` = uniform random pick, `2` = importance-sampled pick). Importance-sampled primary opaque direct lighting uses the standard local RIS reservoir; see `07-shader-lighting-and-materials.md` and `23-initial-ris-direct-lighting-plan.md`.
