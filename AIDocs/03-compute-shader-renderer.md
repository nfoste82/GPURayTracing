...
- `GameManager.enableAdaptiveSampling`: experimental static-final-color progressive path. It assigns each fixed `8x8` group its uniform-equivalent path budget every frame: unpromoted groups spend it on a coarse guide, while promoted groups spend it as one full-resolution path per pixel. A group promotes permanently after two stable guide batches. `adaptiveGuidanceBrightnessPriority`, `adaptiveGuidanceDirectLightPriority`, and `adaptiveGuidanceRoughnessPriority` are signed `-2..2` policy weights: positive values promote brighter, more directly lit, or smoother/specular groups respectively, while negative values deprioritize those signals. The guide records first-surface roughness alongside its existing direct-light signal, without adding a dispatch or persistent resource. Root paths are traced through one indirect dispatch and resolved once per active pixel. `CSMain` remains the unchanged uniform reference when the toggle is off.
