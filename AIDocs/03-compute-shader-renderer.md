...
- `GameManager.enableAdaptiveSampling`: experimental static-final-color progressive path. When eligible, it classifies fixed `8x8` spatial blocks by the mean display-space uncertainty of their valid (in-bounds) pixels, globally redistributes the fixed per-frame path budget through compact pixel and root-path work lists, and uses each pixel's retired path count as its deterministic sample index. Root paths are traced through one indirect dispatch and resolved once per active pixel, avoiding inactive fixed waves. The allocation is reused for `adaptiveSamplingReclassificationInterval` frames (default `8`) to keep scheduler overhead out of the hot progressive path. `CSMain` remains the unchanged uniform reference when the toggle is off.
