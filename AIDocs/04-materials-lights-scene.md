...
`Tools > Ray Tracing > Generate Remote Material Ball Scene` creates `Assets/Scenes/Generated/RemoteMaterialBall.unity`. It recreates the three-gpu-pathtracer material-ball setup with its USD Working Group shader-ball GLB loaded from gkjohnson's external `3d-demo-data` repository. The source scene remains at scale `1` because project units are meters. After download it selects the `material_surface` mesh for a gold, smooth metallic `RayMaterial`, creates the five area lights from the named glTF anchors using this renderer's radiance scale, and copies the embedded camera pose/FOV to the project camera before removing the imported camera GameObject. The scene uses `LightFalloffScale = 0.001` because the reference path tracer treats the loaded rectangular lights as radiance emitters with a different area-light estimator; the project's default inverse-square-style attenuation otherwise makes the material-ball room substantially darker. It also uses this renderer's normal exposure and a low-intensity directional fill because the source's `0.02` three.js exposure relies on its separate physical-light calibration. The external asset is intentionally not stored in this repository; opening this scene in Play mode requires network access to the pinned remote URL.
