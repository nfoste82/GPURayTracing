...
7. Selects accumulation policy. When temporal denoising is enabled and the camera changed beyond its temporal motion thresholds since the prior rendered frame, bounded temporal accumulation is used. Otherwise, final-color progressive accumulation can continue when enabled. Adaptive sampling is optional within progressive accumulation: the GPU keeps per-pixel sample count and luminance variance state, and stable pixels are revisited on staggered 2-, 4-, 8-, or 16-frame intervals. Changing adaptive sampling enablement, thresholds, or presets deliberately does not reset accumulation; when adaptive sampling is enabled after ordinary accumulation, the shader seeds each pixel's adaptive count and mean from the existing global accumulation. Temporal history remains updated while the still-image path is presented, ready for renewed camera movement. Progressive accumulation resets when the render size, camera matrices, focus distance, quality settings other than adaptive sampling policy, random-noise setting, skybox texture/tint, sphere/light data, mesh object transforms/materials, or relevant object counts change. Debug render modes and `enableFrameAccumulation == false` disable progressive accumulation.
...
- `float3 vertex1`
- `float3 vertex2`
- The CPU and HLSL field order is defined by `GameManager.Triangle` and `MeshTriangle`; image regression fixtures mirror the same layout and stride.
...
`UpdateSpheres()` compares current sphere/light structs with their cached values and calls `SetData()` only after a change. Sphere material edits and light radiance edits update their data buffers without rebuilding either top-level tree. Changed regular-sphere bounds mark both top-level BVHs dirty; changed sphere-light or directional-light bounds mark the general top-level BVH dirty.

`UpdateTriangles()` distinguishes geometry changes from material-only changes. Each unique Unity `Mesh` and smooth-normal setting has one cached object-space BVH template in `MeshBvhTemplateCache`. Transform changes reuse that topology and only transform its triangles and node bounds into world space; changing the smooth-normal setting selects or builds the corresponding template. Color, emission, smoothness, metallic, opacity, refraction, material-type, or texture-reference changes update the existing sorted triangle records and texture arrays without touching BVHs. Each texture array uses the largest source width and height in its material channel. Same-size readable textures are copied without resampling; smaller readable textures are resampled into bilinear, repeat-wrapped slices; non-readable assets, including Unity built-in textures, are copied through a temporary render texture. Metallic-roughness and normal arrays are linear data; albedo remains color data. `SceneBvhManager` rebuilds/uploads the general and shadow structures only after the relevant bounds, membership, or activation threshold changes, so static scenes retain their existing GPU trees.

`UpdateSpheres()` and `UpdateMeshChangeCache()` also recompute whether any shadow-casting blocker (regular sphere or mesh triangle) has opacity `< 1`. `SetShaderParameters()` uploads the combined result as the `_HasTransparentShadowBlockers` int so the shader can take its cheaper pure-occlusion shadow path when no transparent blockers exist (see `06-shader-intersections-and-bvh.md`). Caustic grid diagnostics use an asynchronous metadata readback, so overlay counts may represent a recently completed photon batch rather than the batch dispatched in the current frame.
...
Video capture temporarily enables single-frame accumulation without its normal 10 FPS cap, disables temporal denoising, and chooses a fixed per-dispatch sample count that divides the requested samples exactly. Caustics-enabled capture uses one camera sample per dispatch because the photon map advances once per dispatch; this gives every requested video sample its own photon batch instead of packing many camera samples around one batch. Once the requested samples have accumulated, capture synchronously writes the display-ready output as `frame_000000.png`, advances Unity by one deterministic frame using `Time.captureDeltaTime`, pauses again before rendering, and repeats. Spatial denoising can remain enabled as a per-frame post-process. A unique timestamped directory prevents previous captures from being overwritten. After capture, ffmpeg encodes the sequence to H.264 `video.mp4` at `1 / timestep` FPS while retaining the lossless PNG sources; its executable can be configured explicitly or discovered from PATH/common Homebrew locations. Completion, cancellation, write failure, and object destruction restore the render and Unity timing settings that were active before capture.
