...
`UpdateSpheres()` compares current sphere/light structs with their cached values and calls `SetData()` only after a change. Changed sphere bounds mark both top-level BVHs dirty; changed sphere-light bounds mark the general top-level BVH dirty.

`UpdateTriangles()` distinguishes geometry changes from material-only changes. Each unique Unity `Mesh` and smooth-normal setting has one cached object-space BVH template. Transform changes reuse that topology and only transform its triangles and node bounds into world space; changing the smooth-normal setting selects or builds the corresponding template. Color, emission, smoothness, metallic, opacity, refraction, material-type, or texture-reference changes update the existing sorted triangle records and texture arrays without touching BVHs. Each texture array uses the largest source width and height in its material channel. Same-size readable textures are copied without resampling; smaller readable textures are resampled into bilinear, repeat-wrapped slices; non-readable assets, including Unity built-in textures, are copied through a temporary render texture. Metallic-roughness and normal arrays are linear data; albedo remains color data. `UpdateTopLevelBvh()` rebuilds and uploads the top-level object BVH every rendered frame after sphere/light and mesh updates, so dynamic objects keep correct top-level bounds. `UpdateShadowBvh()` does the same for the shadow-only blocker BVH.
