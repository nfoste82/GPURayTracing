...
`GetLightHittingPoint()` computes direct lighting by drawing one or more global emitters per shading point and shading each with stochastic samples across the light shape. Sphere lights use disk samples across the emissive sphere radius. Each mesh light is one global emitter: it selects a triangle with a world-space-area-weighted CDF, then uses a uniform barycentric sample across that triangle. When enabled, one separately sampled environment draw is also processed by this same loop. The combined global-emitter, triangle, and shape PDF is used by MIS, and the direct estimator compensates for triangle selection. Bounce 0 uses `max(1, _ShadowQuality + 1)` samples per shaded light, while later bounces use one sample per shaded light to reduce cost. The actual per-light shading work lives in `SampleSingleLight()`. Shadow-ray offsets select the geometric-normal side containing the sampled light direction, which is necessary when shading an inner glass boundary.
