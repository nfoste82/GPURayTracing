...
`RayDirectionalLight` supplies an analytic directional light. Its transform forward axis is the direction that illumination travels, so direct-light/shadow rays use its negation. Its `Intensity` is HDR radiance without distance falloff. `AngularRadius` is sampled uniformly over the corresponding solid-angle cone, producing soft sun shadows; zero is a hard-shadow delta sun. Directional lights use infinite-distance shadow queries and are outside the current area-light/opaque-BRDF MIS pairing. They do not have emissive scene geometry and are not caustic photon emitters.

Directional lights are internally represented by two virtual triangle emitters at a fixed distance for triangle-light sampling and photon transport. Their direct-light contribution remains distance independent. Caustic photon power compensates for the virtual distance squared, preventing photons from becoming imperceptibly weak merely because the virtual sun is far away.

`GetLightHittingPoint()` computes direct lighting by drawing one or more global emitters per shading point and shading each with stochastic samples across the light shape. Sphere lights use disk samples across the emissive sphere radius. Each mesh light is one global emitter: it selects a triangle with a world-space-area-weighted CDF, then uses a uniform barycentric sample across that triangle. When enabled, one separately sampled environment draw is also processed by this same loop. Eligible primary opaque events use the standard local RIS reservoir over the configured candidate count and route only the selected candidate through the production visibility path. Ineligible events retain ordinary direct sampling. The combined global-emitter, triangle, and shape PDF is used by the reservoir-aware MIS policy, and the direct estimator compensates for triangle selection. The actual per-light shading work lives in `SampleSingleLight()`. Shadow-ray offsets select the geometric-normal side containing the sampled light direction, which is necessary when shading an inner glass boundary.
...
Adaptive guide traces separately accumulate the luminance of these next-event direct-light contributions, including direct fog scattering. `adaptiveGuidanceDirectLightPriority` uses this signal only to promote an `8x8` group to full-resolution sampling sooner; it never changes the path estimator, final-color radiance, or fixed per-frame path budget.
...
Explicit triangle-light samples and opaque BRDF continuation samples are combined with the reservoir-aware power-heuristic multiple importance sampling policy. Triangle area samples are converted to solid-angle PDFs, include the active light-selection probability, and account for RIS candidate/reservoir normalization and the competing technique. Emissive triangle hits reached by an opaque BRDF sample receive the complementary weight. Sphere lights are excluded because their historical falloff-scaled direct-light contribution is not the same estimator as emissive-hit radiance; weighting them as complementary techniques creates dark centers in reflected sphere lights. Glass/water transmission and zero-radius light fallbacks retain their established behavior.
...
- `Diffuse`: uses a mixture of cosine-weighted Lambert sampling and GGX visible-normal reflection sampling. Direct lighting and continuation rays use the same BRDF, roughness mapping, and mixture PDF.
- `Metal`: uses GGX visible-normal reflection sampling with albedo as the Fresnel base reflectance. Direct highlights and continuation reflections therefore share their lobe shape and PDF. Conditioning the microfacet distribution on the current view direction avoids many invalid grazing-angle reflections produced by raw NDF sampling.
...
Opaque GGX continuation samples the Heitz visible-normal distribution and reports its reflected-direction density through the shared BRDF evaluator, so explicit-light and continuation MIS use the same VNDF PDF. Rough glass and water boundary normals intentionally retain their separate raw-GGX/rejection path; replacing dielectric transmission sampling requires an eta-dependent BTDF PDF and is not implied by the opaque reflection sampler.
