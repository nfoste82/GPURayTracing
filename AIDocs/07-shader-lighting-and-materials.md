...
Explicit triangle/environment samples and opaque BRDF continuation samples use power-heuristic weights intended to be complementary. The current implementation has mismatches in RIS technique/count/PDF metadata and final-bounce handling; triangle direct-light distance/falloff conventions also differ from emissive-hit radiance. These are unresolved audit findings, not a validated reservoir-aware MIS policy. Sphere lights are excluded from this pairing because their historical falloff-scaled direct-light contribution is not the same estimator as emissive-hit radiance; weighting them as complementary techniques creates dark centers in reflected sphere lights. Glass/water transmission and zero-radius light fallbacks retain their established behavior. Repair details and acceptance gates belong to document 27.
