...
`RayDirectionalLight` uses transform forward as the direction illumination travels. Zero angular radius uploads two half-radiance analytic directional records, preserving two-slot CPU indexing while producing a hard delta sun with infinite shadow queries. Positive angular radius uses two virtual `SunTriangle` proposals at a fixed distance for direct sampling and photon transport. Those virtual triangles define penumbra directions but are not intersectable emitters, so they remain direct-only and do not receive triangle/BRDF-hit MIS.
...
Direct light from sampled light points is accumulated additively rather than combined with a channel-wise max operation. Sphere-light falloff retains the historical clamped inverse-square-style distance term scaled by light radius and `_LightFalloffScale`. Mesh emitters instead use one-sided radiance with the physical `cos(emitter) * area / distance^2` geometry factor; their direct samples and emissive continuation hits therefore estimate the same integrand. Back-facing or degenerate triangle samples contribute zero rather than falling into the sphere/point fallback. Every surface boundary occludes a straight direct-light connection. Illumination through glass is sampled by dielectric continuation (and optionally the separate caustic photon estimator), not by an unrefracted transparent shadow ray.
...
`GetShadowTransmittance()` is a pure visibility query: the first sphere or mesh boundary before the sampled light returns black. A straight segment through dielectric geometry cannot represent its Snell-refracted continuation or provide a matching path-space PDF, so absorption-only transparent shadows are not treated as an estimator of that transport. Glass color absorption still applies to continuation segments through the active medium.
...
- `Glass`: uses `RayMaterial.Specular` as a minimum reflection chance, then Schlick Fresnel to raise reflection toward one at grazing angles. `RayMaterial.Transmission` independently controls the remaining transmission chance. Sphere and mesh paths sample view-conditioned GGX visible boundary normals from smoothness for both reflection and Snell transmission, so rough glass broadens refractions and caustics into a frosted appearance without the raw-NDF sampler's excess grazing directions. Mesh glass supports bounded internal total internal reflection (TIR) while searching for an exit face. Transmitted glass paths are attenuated by distance-based RGB absorption. Dielectric surfaces are continuation-only until direct reflection/refraction sampling has a matching dielectric evaluate/PDF contract.
