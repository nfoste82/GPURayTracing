...
- Transparent shadows and transparent ray paths add extra math and intersection tests. When a scene has no transparent shadow blockers, shadow rays take a cheaper boolean pure-occlusion path (`_HasTransparentShadowBlockers == 0`), so introducing any transparent blocker (sphere or mesh with opacity `< 1`) switches every shadow ray to the more expensive nearest-transparent-blocker path.
