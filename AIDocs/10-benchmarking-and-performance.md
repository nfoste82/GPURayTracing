...
Use `Tools > Ray Tracing > Precompile Compute Shader` to compile and dispatch all eight `CSMain` debug/fog/terrain combinations from edit mode with timing per combination and surfaced compile messages, so slow or failing kernels appear there instead of stalling on first Play. The submenu also exposes each individual combination and a separate `Caustics` warmup, so use a single variant while iterating on shader changes and reserve `All Variants` for a complete renderer warmup. Each run clears Unity's generated `Library/ShaderCache` data and force-reimports the selected compute asset before dispatching, so the selected variant's first dispatch is intended to be a cold compile; a second dispatch records the warm baseline. The cache clear affects generated local editor data only and can make the next import slower. The tool shows a cancelable editor progress bar with the complete keyword combination and remaining count, and appends each completed row immediately to `Library/RayTracingShaderCompileStats.csv` (the file is intentionally ignored by git). Runtime caustic photon dispatch is disabled during the renderer warmup; use the separate caustics entry when validating that asset. The renderer warmup binds minimal dummy terrain resources, including `_SkyboxTexture`, and disables environment lighting; those runtime resources do not change the shader variant key.
