...
- RIS GPU helper probes cover reservoir normalization and invalid-history rejection. Local initial-RIS image tests vary candidate counts `1`, `2`, `4`, and `8`, check finite mesh-light radiance, and compare untone-mapped triangle/environment/mixed-light fixture means against ordinary NEE plus BRDF-hit estimates at the fixtures' default light/environment sample counts. These are genuine helper/local-estimator tests, not production temporal/spatial history mean-correctness tests or general MIS validation for non-default technique counts, empty outcomes, and terminal bounces.
...
- T1 still requires grazing sampling-frequency integration, white-furnace and raw-HDR mean acceptance, and relevant water/fog/RIS/guiding wrapper compiles. A targeted dry wavefront precompile was run, but its result is still being checked; no success is claimed yet. A broader compute-test-class attempt timed out after 120 seconds without results. No full-suite result or image-baseline update accompanies this repair; the focused results above do not complete Phase 1.
