...
| T5 | P1 | **Repaired with continuation-only dielectric transport for intersectable emitters, environment, and transmission; full dielectric NEE remains future work.** Straight NEE segments no longer pass through dielectric boundaries. Non-intersectable directional lights and virtual sun triangles directly evaluate only the reflected dielectric GGX lobe and do not create competing continuation MIS metadata. Broader delta and rough dielectric direct sampling still requires coherent sample/evaluate/PDF contracts. | `ShouldSampleDirectLight`, `SampleSingleLight`, `GetShadowTransmittance`, `CreateScatteredRay` |
