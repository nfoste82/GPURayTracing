...
Shadow rays traverse the shadow-only blocker BVH when enabled, or flat-loop blockers for small scenes. They test blockers against regular sphere and mesh leaves, but not light leaves. Opaque blockers early-out immediately, while transparent blockers multiply accumulated RGB transmittance before the light distance. Sphere blockers estimate distance through the medium from the ray/sphere chord; mesh blockers currently apply each transparent triangle hit as a thin filter.
