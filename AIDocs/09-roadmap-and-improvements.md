...
1. Add a physically configurable camera aperture. Replace the hard-coded `0.005` three-axis origin jitter with a lens radius and/or f-stop, sample a disk in camera right/up space, preserve an exact pinhole path at zero aperture, reset accumulation when lens settings change, and add deterministic focus-plane coverage. Aperture blade count, rotation, and anamorphic ratio can remain later extensions.
...
4. Extend mesh materials with metallic/roughness and emission textures. Reuse the existing UV/texture-array path, define color-space and missing-map defaults, feed roughness into the existing shared GGX model, and ensure emission textures participate consistently in emissive hits and light construction. Improve texture filtering/resolution and add tangent-space normal maps only after these lower-risk maps are stable.
