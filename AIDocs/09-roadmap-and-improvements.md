...
- **Internal-resolution tracing plus reconstruction: highest general payoff.** Add independent internal render-scale presets of `67%` (quality), `58%` (balanced), and `50%` (performance) for beauty and feature generation. Denoise linear HDR at internal resolution, spatially upscale with Catmull-Rom/CAS/FSR 1 initially, and tone-map only after reconstruction. A `50%` width/height scale traces one quarter of the primary pixels before post-process overhead. Thin silhouettes, fine textures, tiny specular highlights, and focused caustics are the principal risks. See `13-denoising-and-upscaling.md` for the required resource and testing contract.
- **One stochastic area-light shadow sample at primary hits.** The current first bounce takes `max(1, shadowQuality + 1)` shadow samples per selected light. Add an interactive preset that uses one sample (`shadowQuality = 0`) and relies on temporal accumulation/denoising for convergence. This removes two of the three default primary shadow traversals. The tradeoff is noisier or more temporally unstable soft-shadow penumbrae; compare static accumulation, camera motion, contact shadows, and large area lights before enabling it dynamically. A later refinement can rotate one area-light sample per frame rather than spending several samples in one frame.
- **Importance-selected lights for many-light scenes.** For scenes where direct-light loops dominate, prefer `ImportanceSampled` with one or a small number of `lightSampleCount` draws rather than `AllLights`. This is already available and should remain a dynamic-quality option, not a universal default: it trades deterministic per-frame lighting for Monte Carlo noise and currently becomes biased above `MaxImportanceLights`. The selected direct-light PDF is now returned by the selection helper, avoiding the previously redundant post-selection importance scan. Benchmark `Benchmark_ManyLights` at matched sample counts and compare variance as well as frame time.
- **Lower-frequency autofocus.** Center autofocus currently traces CPU scene geometry every rendered frame. Update only while camera/scene state changes and otherwise reuse the last focal distance; a `10-20 Hz` update cadence should be visually acceptable because focus is already smoothed. Prefer the production GPU focus query at a bounded asynchronous cadence after confirming latency behavior. Reuse traversal storage and/or the CPU top-level BVH if retaining a CPU fallback. Validate camera motion, fast foreground crossings, glass opacity policy, water, and click-to-focus behavior.
